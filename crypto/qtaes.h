#ifndef QTAES_H
#define QTAES_H // thanks2 bricke/Qt-AES@github
#include <QObject>
#include <QByteArray>
class QtAes : public QObject{ Q_OBJECT Q_DISABLE_COPY(QtAes) public:
 enum Aes     { AES_128, AES_192, AES_256 };
 enum Mode    { ECB,     CBC,     CFB     };
 enum Padding { ZERO,    PKCS7,   ISO     };
 QtAes(Aes level, Mode mode, Padding padding = QtAes::ISO)
    : m_nb(4), m_blocklen(16), m_level(level), m_mode(mode), m_padding(padding)
{
    m_state = NULL;
    switch (level){
    case AES_128: { AES128 aes;     m_nk = aes.nk; m_keyLen = aes.keylen;
                    m_nr = aes.nr;  m_expandedKey = aes.expandedKey;
                  } break;
    case AES_192: { AES192 aes;     m_nk = aes.nk; m_keyLen = aes.keylen;
        m_nr = aes.nr;        m_expandedKey = aes.expandedKey;
                  } break;
    case AES_256: { AES256 aes;     m_nk = aes.nk; m_keyLen = aes.keylen;
        m_nr = aes.nr;        m_expandedKey = aes.expandedKey;
                  } break;
    default: {      AES128 aes;     m_nk = aes.nk; m_keyLen = aes.keylen;
        m_nr = aes.nr;        m_expandedKey = aes.expandedKey;
                  } break;
    }
}
...
QByteArray getPadding(int currSize, int alignment)
{
    ...
    switch(m_padding) {
    case Padding::ZERO: ret.insert(0, size, 0x00); break;
    case Padding::PKCS7: ret.insert(0, size, size); break;
    case Padding::ISO: ret.insert(0, 0x80);
                      ret.insert(1, size, 0x00);        break;
    default:        ret.insert(0, size, 0x00);    break;
    }
    return ret;
}
 ...
 * Static Functions
 * */
QByteArray Crypt(Aes level,
                 Mode mode,
                 const QByteArray &rawText,
                 const QByteArray &key,
                 const QByteArray &iv = NULL,
                 Padding padding = QtAes::ISO)
  {return QtAes(level, mode, padding).encode(rawText, key, iv);}

QByteArray Decrypt(Aes level,
                   Mode mode,
                   const QByteArray &rawText,
                   const QByteArray &key,
                   const QByteArray &iv = NULL,
                   Padding padding = QtAes::ISO)
 {return QtAes(level, mode, padding).decode(rawText, key, iv);}

//
QByteArray ExpandKey(Aes level, Mode mode, const QByteArray &key)
 {return QtAes(level, mode).expandKey(key);}
//
QByteArray RemovePadding(const QByteArray &rawText, Padding padding)
{
